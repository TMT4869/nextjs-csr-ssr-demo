...
import { BlogPost, searchBlogs, filterBlogsByCategory, categories } from '@/lib/blog-utils';
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-center py-12">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-gray-600 dark:text-gray-400">Đang lọc kết quả...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Hiển thị {filteredBlogs.length} / {initialBlogs.length} bài viết
            {searchQuery && (
            ...
            {categoryFilter !== 'all' && (
              <span className="ml-2 px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded text-sm">
                Danh mục: {getCategoryInfo(categoryFilter)?.name}
              </span>
            )}
          </p>
        </div>
            ...
            >
              ...
              <div className="relative overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  {categoryInfo && (
                    <span className={`px-3 py-1 text-xs font-medium rounded-full ${getCategoryColor(categoryInfo.color)}`}>
                      {categoryInfo.name}
                    </span>
                  )}
                </div>
      ...
      {filteredBlogs.length > 0 && (
        <div className="text-center mt-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
            (Sẽ sử dụng CSR để fetch thêm dữ liệu)
          </p>
        </div>
      )}
    </div>
